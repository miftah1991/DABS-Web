...
Class Tender extends CI_Controller{
    ...
    function ListTenders(){
	 ...
	 function SearchTenders(){
		...
		if($search){
					...
					$sql = $this->db->query("SELECT * FROM tndr_tbl where t_specification_number='$search'");
					 $this->load->view('admin/tender_list_s', $sql->result_array());
    ...
    function validate(){
			...
			$this->form_validation->set_rules('t_title', 'Description','required|trim|xss_clean|strip_tags');
