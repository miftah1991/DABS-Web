...
Class New_m  extends CI_Model {
    ...
    function GetAll(){
        $q = $this->db->query("SELECT n.*,l.language FROM tbl_news n, tbl_lng l where n.language_id=l.language_id order by n.date desc");
        return $q->result();
    }
    ...
    function UpdateArticle($nid,$inputArray){
        $this->db->where('nid', $nid);
            $this->db->update('tbl_articles', $inputArray);
    }
